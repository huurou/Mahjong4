...
/// </summary>
public sealed class AI_v0_3_0_評価値Factory(int seed) : IPlayerFactory
{
    public string DisplayName => AI_v0_3_0_評価値.DISPLAY_NAME;

    public Player Create(PlayerIndex index, PlayerId id)
    {
        return new AI_v0_3_0_評価値(id, index, new Random(DeriveSeed(seed, index.Value)));
    }
