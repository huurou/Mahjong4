...
public class AI_v0_5_0_鳴き_OnKanTsumoTests
{
    [Fact]
    public async Task RinshanTsumoAgariCandidateあり_RinshanTsumoResponseを返す()
    {
        ...
        var notification = new KanTsumoNotification(
    ...
    [Fact]
    public async Task 通常手牌_KanTsumoDahaiResponseを返す()
    {
        // Arrange
    ...
    [Fact]
    public async Task 役ありシャンテン維持する暗槓候補_KanTsumoAnkanResponseを返す()
    {
        ...
        // Assert
        var ankan = Assert.IsType<KanTsumoAnkanResponse>(response);
        Assert.Equal(TileKind.Haku, ankan.Tile.Kind);
    }

    [Fact]
    public async Task 他家リーチ中かつ非テンパイ_暗槓候補あり_KanTsumoDahaiResponseを返す()
    {
        // Arrange: Haku 4 枚手牌だが非テンパイ + 他家リーチ中。カンを避けて嶺上ツモ後の打牌のみ返す。
        var player = await CreateAIWithGameStartAsync();
        var hand = BuildHand(
